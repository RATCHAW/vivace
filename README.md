...
- A Strava-authenticated overview with season totals, the latest activities and
  athlete details.
- A runs catalogue with a linkable 9:16 route replay, live pace and heart-rate
  metrics, an optional athlete avatar and a theatre view.
...
`packages/video/src/templates/`, and entries in the component/default-props maps
in `Root.tsx` — and **no change to apps/api at all**. A test fails if the
registry and the components drift.
...
template renders. Today that's 1.1MB entry against 10MB lazy — Mapbox is not in
...
# hover-card  input  input-group  label  progress  separator  skeleton  slider
# sonner  spinner  switch  textarea  tooltip
```
