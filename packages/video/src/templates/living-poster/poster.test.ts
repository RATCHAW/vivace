...
describe("posterRoute", () => {
  ...
  it("drops the fix that bounced off a building", () => {
    ...
    const worst = Math.max(
      ...cleaned
        .slice(1)
        .map((point, index) => distanceMeters(cleaned[index], point)),
    );
...
describe("the composition", () => {
  it("frames the route in the upper band, north up, inside its padding", () => {
    for (const fixture of [FIXTURE_A, FIXTURE_E, FIXTURE_F]) {
      ...
      expect(Math.min(...xs), fixture.key).toBeGreaterThanOrEqual(
        POSTER_PADDING.left - 0.5,
      );
      expect(Math.max(...xs), fixture.key).toBeLessThanOrEqual(
        CANVAS_WIDTH - POSTER_PADDING.right + 0.5,
      );
      expect(Math.min(...ys), fixture.key).toBeGreaterThanOrEqual(
        POSTER_PADDING.top - 0.5,
      );
      ...
      expect(CANVAS_HEIGHT - POSTER_PADDING.bottom).toBeLessThan(
        TYPE_BLOCK_TOP,
      );
    }
  });
  ...
  it("scales the stroke to the route, so a loop and a marathon read as one pen", () => {
    const loop = posterPlan(
      FIXTURE_A.activity,
      FIXTURE_A.streams,
      FPS,
      10 * FPS,
    );
    const trail = posterPlan(
      FIXTURE_E.activity,
      FIXTURE_E.streams,
      FPS,
      10 * FPS,
  ...
  it("gives elevation a column only when there was a hill", () => {
    ...
    expect(posterStats(FIXTURE_E.activity)[3]).toMatchObject({
      label: "Elevation",
      unit: "m",
    });
  });
  ...
  it("draws the same poster twice", () => {
    ...
    ).toEqual(posterPlan(FIXTURE_A.activity, FIXTURE_A.streams, FPS, 10 * FPS));
  });
});
